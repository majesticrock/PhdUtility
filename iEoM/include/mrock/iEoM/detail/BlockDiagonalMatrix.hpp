...
using diagonal_segment_type =
    decltype(std::declval<Eigen::DiagonalWrapper<_matrix>>().diagonal().segment(0, 1).asDiagonal());
 ...
 */
template <class EigenMatrixType>
std::vector<HermitianBlock> identify_hermitian_blocks(const EigenMatrixType& matrix,
                                                      const RealScalar<EigenMatrixType> epsilon = 1e-12) {
...
struct base_traits {
    ...
    using as_is_vector =
        std::conditional_t<std::is_const_v<_matrix_base>, std::add_const_t<mutable_vector>, mutable_vector>;
};
...
struct BlockDiagonalMatrix {
     ...
     */
    BlockDiagonalMatrix(typename base_traits<_matrix_base>::mutable_vector&& _blocks,
                        std::vector<Eigen::Index> const& _blocks_begin)
...
#pragma omp parallel for
     ...
     */
    inline const InternalBase& block(std::size_t i) const { return blocks[i]; }
    /**
     * @brief Access a block by index.
     *
     * @param i Zero-based block index.
     ...
     */
    inline InternalBase& block(std::size_t i) { return blocks[i]; }
 ...
 */
template <class _base, class _other_base>
BlockDiagonalMatrix<addition_result<_base, _other_base>> operator+(BlockDiagonalMatrix<_base> const& lhs,
                                                                   BlockDiagonalMatrix<_other_base> const& rhs) {
 ...
 */
template <class _base, class _other_base>
BlockDiagonalMatrix<substraction_result<_base, _other_base>> operator-(BlockDiagonalMatrix<_base> const& lhs,
                                                                       BlockDiagonalMatrix<_other_base> const& rhs) {
 ...
 */
template <class _base, class _other_base>
BlockDiagonalMatrix<multiplication_result<_base, _other_base>> operator*(BlockDiagonalMatrix<_base> const& lhs,
                                                                         BlockDiagonalMatrix<_other_base> const& rhs) {
...
BlockDiagonalMatrix<addition_result<_matrix_base, diagonal_segment_type<__matrix__>>> operator+(
    BlockDiagonalMatrix<_matrix_base> const& lhs,
    ...
    return BlockDiagonalMatrix<addition_result<_matrix_base, diagonal_segment_type<__matrix__>>>(std::move(new_blocks),
                                                                                                 lhs.blocks_begin);
}
 ...
 */
template <class _matrix_base, class __matrix__>
BlockDiagonalMatrix<substraction_result<_matrix_base, diagonal_segment_type<__matrix__>>> operator-(
    BlockDiagonalMatrix<_matrix_base> const& lhs,
    Eigen::DiagonalWrapper<__matrix__> const& rhs) {
    ...
    return BlockDiagonalMatrix<substraction_result<_matrix_base, diagonal_segment_type<__matrix__>>>(
        std::move(new_blocks), lhs.blocks_begin);
}
 ...
 */
template <class _matrix_base, class __matrix__>
BlockDiagonalMatrix<multiplication_result<_matrix_base, diagonal_segment_type<__matrix__>>> operator*(
    BlockDiagonalMatrix<_matrix_base> const& lhs,
    Eigen::DiagonalWrapper<__matrix__> const& rhs) {
    ...
    return BlockDiagonalMatrix<multiplication_result<_matrix_base, diagonal_segment_type<__matrix__>>>(
        std::move(new_blocks), lhs.blocks_begin);
}
}  // namespace mrock::iEoM::detail
