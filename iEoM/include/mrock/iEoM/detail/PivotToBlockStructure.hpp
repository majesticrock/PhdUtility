...
Eigen::PermutationMatrix<Eigen::Dynamic, Eigen::Dynamic> pivot_to_block_structure(
    const EigenMatrixType& matrix,
    const detail::RealScalar<EigenMatrixType> epsilon = 1e-12) {
...
template <class MatrixType,
          class RealType = UnderlyingRealType_t<typename MatrixType::Scalar>,
          class RealVector = Eigen::Vector<RealType, Eigen::Dynamic>>
struct matrix_wrapper {
     ...
     */
    static matrix_wrapper solve_block_diagonal_matrix(const MatrixType& toSolve,
                                                      const std::vector<HermitianBlock>& blocks) {
...
#pragma omp parallel for
        ...
        for (std::size_t i = 0; i < blocks.size(); ++i) {
            ...
            solution.eigenvectors.block(blocks[i].position, blocks[i].position, blocks[i].size, blocks[i].size) =
                solver.eigenvectors();
        }
    ...
    static bool is_non_negative(MatrixType& toSolve, const RealType EPSILON) {
        ...
        for (const auto& block : blocks) {
            Eigen::SelfAdjointEigenSolver<MatrixType> solver(
                toSolve.block(block.position, block.position, block.size, block.size), Eigen::EigenvaluesOnly);
...
struct matrix_wrapper<BlockDiagonalMatrix<detail::MatrixN<Number>>,
                      UnderlyingRealType_t<Number>,
                      Eigen::Vector<UnderlyingRealType_t<Number>, Eigen::Dynamic>> {
     ...
     */
    inline auto reconstruct_matrix() const { return eigenvectors * eigenvalues.asDiagonal() * eigenvectors.adjoint(); }
    /**
...
template <class Number>
using blocked_matrix_wrapper = matrix_wrapper<BlockDiagonalMatrix<detail::MatrixN<Number>>,
                                              UnderlyingRealType_t<Number>,
                                              Eigen::Vector<UnderlyingRealType_t<Number>, Eigen::Dynamic>>;
}  // namespace mrock::iEoM::detail
