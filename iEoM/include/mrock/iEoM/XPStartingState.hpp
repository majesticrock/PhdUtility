...
struct XPStartingState {
     ...
     */
    inline bool contains_amplitude_state() const noexcept { return amplitude_state.size() != 0; }

    /**
     * @brief Returns true when a phase state vector is present.
     * @return true if phase_state is non-empty.
     */
    inline bool contains_phase_state() const noexcept { return phase_state.size() != 0; }
...
int total_size(std::vector<XPStartingState<RealType>> const& states) {
    return std::accumulate(
        states.begin(), states.end(), int{},
        [](int current, const XPStartingState<RealType>& state) { return std::move(current) + state.size(); });
}
}  // namespace mrock::iEoM
