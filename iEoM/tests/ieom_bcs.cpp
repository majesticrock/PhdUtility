...
constexpr double U =
    -2;  // Attractive interaction; larger U are easier numerically, and this is just a test script anyways
...
struct BCSTester : public XPResolvent<double, 1, true> {
    ...
    double E(double eps) { return std::sqrt(eps * eps + Delta * Delta); }

    // Expectation value of the number operator in BCS mean-field.
    double number(double eps) { return 0.5 * (1. - eps / E(eps)); }

    // Expectation value of the pair operator in BCS mean-field.
    double pair(double eps) { return -0.5 * Delta / E(eps); }
    ...
    void fill_M() override {
        ...
        for (int k = 0; k < N_k; ++k) {
            ...
            for (int l = 0; l < N_k; ++l) {
                ...
                K_minus(l, k) +=
                    (2 * U / N_k) * (1 - 2 * number_k) * (1 - 2 * number_l) + (8 * U / N_k) * pair_l * pair_k;
            }
        }
    }
...
std::array<std::string, N_lanczos> save_data(const std::vector<BCSTester::ResolventReturnData>& data,
                                             std::ofstream& out) {
...
int main() {
    ...
    std::cout << "Successfully detected negative eigenvalues in the dynamical matrix for a non-equilibrium gap value."
              << std::endl;
