...
Real printer(Real ana, Real num, std::string const& algorithm) {
    std::cout << algorithm << ": \t" << "Analytical: " << ana << " Numerical: " << num
              << " Error: " << std::abs(ana - num) << std::endl;
...
result_set_A get_generalized_numerics(Real a, Real b, Real c) {
    ...
    const std::vector<Real> second_singularitites =
        (c != 0 ? std::vector<Real>{std::min(0.0, c), std::max(c, 0.0)} : std::vector<Real>{0.0});
    return {mrock::utility::Numerics::Integration::GeneralizedPrincipalValue<Real, N>::generalized_principal_value(
                f1, a, b, std::vector<Real>{c}),
            mrock::utility::Numerics::Integration::GeneralizedPrincipalValue<Real, N>::generalized_principal_value(
                f2, a, b, second_singularitites),
            mrock::utility::Numerics::Integration::GeneralizedPrincipalValue<Real, N>::generalized_principal_value(
                f3, a, b, std::vector<Real>{c}),
            mrock::utility::Numerics::Integration::GeneralizedPrincipalValue<Real, N>::generalized_principal_value(
                f4, a, b, std::vector<Real>{c})};
}

result_set_A get_analytics(Real a, Real b, Real c) {
    return {std::log(std::abs(c - b) / std::abs(c - a)),
            (c != Real{0} ? std::log(std::abs(c - b) / std::abs(c - a)) / (c * c) -
                                std::log(std::abs(b / a)) / (c * c) + (1. / b - 1. / a) / c
                          : 0.5 * (1. / (a * a) - 1. / (b * b))),
...
int main() {
    ...
    std::cout << "\n###############################\n" << std::endl;
    {
        ...
        auto ana1 = [](Real a, Real b, Real c) {
            return (std::log(std::abs((a + c) / (a - c))) - std::log(std::abs((b + c) / (b - c)))) / (2 * c);
        };
        auto ana2 = [](Real a, Real b, Real c, Real d) {
            return (std::log(std::abs((b - d) / (b - c))) - std::log(std::abs((a - d) / (a - c)))) / (d - c);
        };
        auto ana3 = [](Real a, Real b) { return std::log(std::abs(std::tan(b / 2) / std::tan(a / 2))); };
        ...
        result_set_A numerical_results = {
            ...
            gen_integrator::generalized_principal_value(f3, a, b,
                                                        std::vector<Real>{-2 * M_PI, -M_PI, 0, M_PI, 2 * M_PI}),
            // Computing these kinds of logarithmic singularities is incredibly difficult, but possible
            mrock::utility::Numerics::Integration::GeneralizedPrincipalValue<Real, 10000>::generalized_principal_value(
                f4, a, b, std::vector<Real>{0.5 * (c - 1), 0.5 * (c + 1)})};
        result_set_A analytical_results = {ana1(a, b, c), ana2(a, b, c, d), ana3(a, b),
        ...
        numerical_results = {
            ...
            gen_integrator::generalized_principal_value(
                f3, a, b,
                std::vector<Real>{-4 * M_PI, -3 * M_PI, -2 * M_PI, -M_PI, 0, M_PI, 2 * M_PI, 3 * M_PI, 4 * M_PI}),
            // Computing these kinds of logarithmic singularities is incredibly difficult, but possible
            mrock::utility::Numerics::Integration::GeneralizedPrincipalValue<Real, 10000>::generalized_principal_value(
                f4, a, b, std::vector<Real>{0.5 * (c - 1), 0.5 * (c + 1)})};
        analytical_results = {ana1(a, b, c), ana2(a, b, c, d), ana3(a, b), 7.058322126020065070227595524877212606453};
    ...
    std::cout << "\n###############################\n" << std::endl;
    {
        ...
        /*
         * This is a truly difficult case, but it seems that the issue is actually the normal integration and not the
         * principal value because setting the integration limits to similar magnitude (i.e. computing mainly the PV)
         * yields very good results even for small polynomial degrees N
         */
        a = -1e-8;
        b = 1e-2;
        c = 0;
