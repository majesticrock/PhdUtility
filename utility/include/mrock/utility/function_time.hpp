#ifndef MROCK_UTILITY_INCLUDE_MROCK_UTILITY_FUNCTION_TIME_HPP
#define MROCK_UTILITY_INCLUDE_MROCK_UTILITY_FUNCTION_TIME_HPP
#include <chrono>
#include <iostream>
#include <utility>
...
auto function_time_ms(const FunctionType& func, Args&&... args) -> decltype(func(std::forward<Args>(args)...)) {
    clock::time_point begin = clock::now();
    if constexpr (!(std::is_void_v<decltype(func(std::forward<Args>(args)...))>)) {
        auto return_value = func(std::forward<Args>(args)...);
        clock::time_point end = clock::now();
        std::cout << "Runtime = " << std::chrono::duration_cast<std::chrono::milliseconds>(end - begin).count()
                  << "[ms]" << std::endl;
        return return_value;
    } else {
...
// Prints the runtime of a function to the console and returns the function's return value
template <class FunctionType, class... Args>
auto function_time_micro(const FunctionType& func, Args&&... args) -> decltype(func(std::forward<Args>(args)...)) {
    clock::time_point begin = clock::now();
    if constexpr (!(std::is_void_v<decltype(func(std::forward<Args>(args)...))>)) {
        auto return_value = func(std::forward<Args>(args)...);
        clock::time_point end = clock::now();
        std::cout << "Runtime = " << std::chrono::duration_cast<std::chrono::microseconds>(end - begin).count()
...
#endif  // MROCK_UTILITY_INCLUDE_MROCK_UTILITY_FUNCTION_TIME_HPP
