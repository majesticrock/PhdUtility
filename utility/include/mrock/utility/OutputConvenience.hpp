...
namespace mrock::utility {
	...
	void save_data(const vector_type& data, const std::string& filename,
		const std::vector<std::string>& comments = std::vector<std::string>())
	{
		...
		if (ofile.is_open()) {
			...
			ow.save_data(data, oss, comments);
			out << oss.str();
	...
	template <typename vector_type, typename data_type = typename vector_type::value_type>
	void save_data(const std::vector<vector_type>& data, const std::string& filename,
		const std::vector<std::string>& comments = std::vector<std::string>())
	{
		// create file
		std::ofstream ofile(filename, std::ios_base::out | std::ios_base::binary);
	...
	template <typename vector_type, typename data_type = typename vector_type::value_type>
	void save_data(const vector_type& first, const vector_type& second, const std::string& filename,
		const std::vector<std::string>& comments = std::vector<std::string>())
	{
		...
		const std::vector<std::string>& comments = std::vector<std::string>())
	{
		...
		if (out.is_open()) {
			...
			ow.save_data(data, out, comments);
		}
		else {
			std::cerr << "Could not open output filestream for file: " << filename << std::endl;
		}
