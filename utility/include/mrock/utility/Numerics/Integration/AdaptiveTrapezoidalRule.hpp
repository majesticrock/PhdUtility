...
template <class RealType = double,
          adapative_trapezoidal_rule_print_policy print_steps = adapative_trapezoidal_rule_print_nothing>
struct adapative_trapezoidal_rule {
private:
    /**
    ...
    using error_result =
        std::invoke_result_t<ErrorFunction, decayed_result<UnaryFunction>, decayed_result<UnaryFunction>>;
...
public:
    ...
    decayed_result<UnaryFunction> integrate(
        const UnaryFunction& function,
        const RealType begin,
        const RealType end,
        unsigned int num_steps,
        const error_result<UnaryFunction, ErrorFunction> max_error,
        const ErrorFunction& error_func = ErrorFunction(),
        const decayed_result<UnaryFunction>& zero = decayed_result<UnaryFunction>{}) const {
        throw_exception<std::domain_error>(MROCK_NDEBUG_CONDITION(!(std::isfinite(begin) && std::isfinite(end))),
                                           "The integration domain is not sensible!");
        using std::abs;
        if (abs(begin - end) < std::numeric_limits<RealType>::epsilon())
            return zero;
        if (end < begin) {
        ...
        do {
            ...
            if constexpr (print_steps.print_error) {
                std::cout << "\n\t||\terror = " << current_error << "\tCurrent step = " << step
                          << "\tCurrent number of steps = " << num_steps << std::endl;
            }

            old_value = new_value;
        } while (current_error > max_error);
        if constexpr (print_steps.print_final_state) {
            std::cout << "Final state: Error = " << current_error << "\tCurrent step = " << step
                      << "\tCurrent number of steps = " << num_steps << std::endl;
        }
        return new_value;
    }
    ...
    decayed_result<UnaryFunction> split_integrate(
        const UnaryFunction& function,
        const RealType begin,
        const RealType end,
        unsigned int num_steps_per_interval,
        const error_result<UnaryFunction, ErrorFunction> max_error_per_interval,
        const ErrorFunction& error_func = ErrorFunction(),
        const decayed_result<UnaryFunction>& zero = decayed_result<UnaryFunction>{}) const {
        throw_exception<std::domain_error>(MROCK_NDEBUG_CONDITION(!(std::isfinite(begin) && std::isfinite(end))),
                                           "The integration domain is not sensible!");
        ...
        if (abs(begin - end) < std::numeric_limits<RealType>::epsilon())
            return zero;
        if (end < begin) {
            return -split_integrate<N>(function, end, begin, num_steps_per_interval, max_error_per_interval, error_func,
                                       zero);
        }
        ...
        for (int i = 0; i < N; ++i) {
            if constexpr (print_steps.print_final_state) {
                std::cout << "Interval #" << i << " = [" << begin + i * interval_size << ", "
                          << begin + (i + 1) * interval_size << "]: ";
            }
