#ifndef MROCK_UTILITY_INCLUDE_MROCK_UTILITY_NUMERICS_INTEGRATION_GENERALIZEDPRINCIPALVALUE_HPP
#define MROCK_UTILITY_INCLUDE_MROCK_UTILITY_NUMERICS_INTEGRATION_GENERALIZEDPRINCIPALVALUE_HPP
...
private:
    ...
    using result_type = std::common_type_t<decltype(std::declval<F>()(Real{})), decltype(std::declval<G>()(Real{}))>;

    template <class F>
    static decltype(std::declval<F>()(Real{})) __pv(F const& f,
                                                    Real const& symmetrized_bound,
                                                    Real const& singularity) {
...
public:
    ...
    // Assumes that the zeros are ordered and that the first order Taylor expansion of g(x) is a good approximation near
    // the zeros taylor_tolerance is the distance from the zero where the Taylor expansion is used RegularQuadrature is
    // used for the regular part of the integration. It requires the syntax RegularQuadrature::integrate(f, lower,
    // upper).
    template <class RegularQuadrature, class F, class G, class RandomAccessContainer>
    static result_type<F, G> principal_value_f_over_g(F const& f,
                                                      G const& g,
                                                      Real lower,
                                                      Real upper,
                                                      RandomAccessContainer const& zeros,
                                                      RandomAccessContainer const& g_prime_at_zeros,
                                                      Real taylor_tolerance = 1e-3) {
        if (lower > upper) {
            return -principal_value_f_over_g(f, g, upper, lower, zeros, g_prime_at_zeros, taylor_tolerance);
        }
        result_type<F, G> result{};

        auto integrand = [&f, &g](const Real& x) { return f(x) / g(x); };

        for (int i = 0; i < zeros.size(); ++i) {
            result += RegularQuadrature::integrate(integrand, lower, zeros[i] - taylor_tolerance);
            result +=
                cauchy::cauchy_principal_value(f, zeros[i] - taylor_tolerance, zeros[i] + taylor_tolerance, zeros[i]) /
                g_prime_at_zeros[i];
    ...
    // If one of the bounds of integration is close to the singularity, the other one should be too to guarentee
    // numerical stability!
    template <class F>
    static decltype(std::declval<F>()(Real{})) generalized_principal_value(F const& f,
                                                                           Real lower,
                                                                           Real upper,
                                                                           Real singularity) {
        if (lower > upper) {
...
#endif  // MROCK_UTILITY_INCLUDE_MROCK_UTILITY_NUMERICS_INTEGRATION_GENERALIZEDPRINCIPALVALUE_HPP
