...
protected:
    ...
    template <class F>
    static decltype(std::declval<F>()(Real{})) __cpv(F const& f,
    ...
    static decltype(std::declval<F>()(Real{})) __regular(F const& f, Real lower, Real upper, Real singularity) {
        ...
        auto proper_integrand = [&f, &singularity](const Real& x) { return f(x + singularity) / x; };
        return gauss::integrate(proper_integrand, lower, upper);
    }

public:
    template <class F>
    static decltype(std::declval<F>()(Real{})) cauchy_principal_value(F const& f,
                                                                      Real lower,
                                                                      Real upper,
                                                                      Real singularity) {
