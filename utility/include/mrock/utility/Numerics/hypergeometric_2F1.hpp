...
/**
 * Analytical continuation as a power series from
 * https://functions.wolfram.com/HypergeometricFunctions/Hypergeometric2F1/02/02/
 *
...
auto hg_2F1_continuation(real a, real b, real c, z_t z, real const& tol) {
    ...
    // The rising Pochhammer symbol is defined as (x)_n = Gamma(x + n) / (Gamma(x)) ~> (x+n)! / x! ~>
    // x(x+1)(x+2)...(x+n-1)(x+n) It can thus be computed as (x)_n+1 = (x + n)(x)_n
    ...
    const z_t negative_z = {-std::real(z),
                            std::imag(z) == typename z_t::value_type{} ? typename z_t::value_type{} : -std::imag(z)};
 ...
 * If higher precisions are needed use long double. If that doesn't help, I need to split the summation as to avoid
 * adding 1 + 1e-10 a million times and getting bad results.
 */
template <std::floating_point real, Complex z_t>
auto hypergeometric_2F1(real a, real b, real c, z_t z, real const& tol = real{1e-10}) {
    using std::abs;

    throw_exception<std::invalid_argument>((is_integer(c) && c < real{0}) || abs(c) < tol,
                                           "c must neither be a negative integer nor 0!");
