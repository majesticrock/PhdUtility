...
public:
    explicit NoRootException(const std::string& algorithm, const std::string& function_id)
        : std::invalid_argument("There is no root in the given interval! Encountered in " + algorithm + " using " +
                                function_id) {}
};
...
RealType bisection(const Function& function, RealType begin, RealType end, RealType tol, int maxiter) {
    const auto is_zero = [](RealType val) { return std::abs(val) <= std::numeric_limits<RealType>::epsilon(); };
    ...
    if (maxiter < 0) {
        std::cerr << "Bisection terminated by maxiter-constraint! Function type:" << typeid(Function).name()
                  << std::endl;
    }
