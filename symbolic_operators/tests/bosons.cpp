...
int main(int argc, char** argv) {
    ...
    const Term to_commute_1(1, std::vector<Operator>({Operator::Boson(Momentum('l'), Index::TypeA, true),
    ...
    if (is_baseline)
        tester.save_as_comparison(file_names[0], result_1);
    if (!tester.load_and_test(file_names[0], result_1))
        return 1;

    if (is_baseline)
        tester.save_as_comparison(file_names[1], result_2);
    if (!tester.load_and_test(file_names[1], result_2))
        return 1;
