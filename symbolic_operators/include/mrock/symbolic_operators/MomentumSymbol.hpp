...
template <typename T>
inline std::string _vector_wrap(const T& x)
    requires MrockHasToString<T> && (!MrockValidForVectorWrap<T>)
{
 ...
 */
inline std::ostream& operator<<(std::ostream& os, const MomentumSymbol::name_type name) {
    return (os << _vector_wrap(name));
}
 ...
 */
inline std::istream& operator>>(std::istream& is, MomentumSymbol::name_type& name) {
    return (is >> name._n);
}

/**
 * @brief Concatenates a std::string and a MomentumSymbol::name_type, formatting the latter with vector notation before
 * concatenation.
...
/**
 * @brief Concatenates a MomentumSymbol::name_type and a std::string, formatting the former with vector notation before
 * concatenation.
 * @param sym The MomentumSymbol::name_type whose formatted representation is concatenated with `str`.
