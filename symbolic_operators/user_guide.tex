...
\begin{itemize}
  ...
  \item The Kronecker deltas for indices, stored in \tokencode{std::vector<\classlink{KroneckerDelta}<\classlink{Index}>{}> \detokenize{delta_indices}}
...
\begin{itemize}
  ...
  \item The list of momenta, stored in \tokencode{\classlink{MomentumList} momenta}
  \item The index wrapper, stored in \tokencode{\classlink{IndexWrapper} indices}
...
The attributes are
\begin{itemize}
  \item The wrapped list of indices, stored in \code{indices}
\end{itemize}
...
It supports addition and subtraction-like algebra, and helper functions for sorting, flipping, and removing contributions.
The special momentum $\Pi$ (in some references, the symbol $\mathbf{Q}$ is used instead) can be toggled via the \code{add_PI} attribute.
It has the property $2\Pi = 0$, for instance, on a square lattice, $\Pi = (\pi, \pi)^\mathrm{T}$.
...
\begin{itemize}
  ...
  \item The flag indicating whether the special momentum $\Pi$ is present, stored in \code{add_PI}
\end{itemize}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\begin{itemize}
  ...
  \item The associated indices, stored in \code{indices}
...
\begin{enumerate}
  \item Define symbolic operator strings with \class{Term}.
    ...
    The first term represents a bilinear hopping contribution, and the second one encodes the pairing interaction with a coefficient \code{g(K,P)}.
    ...
    \begin{lstlisting}
const Term H_Kin(1,
    Coefficient("\\epsilon", Momentum('K')),
    SumContainer{ MomentumSum({ 'K' }), Index::Sigma },
    std::vector<Operator>({
...
const Term H_Ph(-1,
    ...
    SumContainer{ MomentumSum({ 'P', 'K' }) },
    std::vector<Operator>({
        c_k_dagger.with_momentum('K'),
        c_minus_k_dagger.with_momentum('K'),
  ...
  \item Define the contraction patterns that Wick's theorem should recognize.
    ...
    \end{lstlisting}
    and one for pair operators of the form $c_{k\uparrow}^{\dagger} c_{-k\downarrow}^{\dagger}$ and $c_{-k\downarrow} c_{k\uparrow}$.
    \begin{lstlisting}
    WickOperatorTemplate{
...
wicks.clean_up();
\end{lstlisting}
    The result is typically messy, and should be cleaned up by calling \code{clean_up}.
